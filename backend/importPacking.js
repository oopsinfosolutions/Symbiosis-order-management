...
const files = [
  path.join(__dirname, "NEW PACKING SHEET (12).xlsx"),
...
async function importExcelData() {
  try {
    ...
    for (const row of combinedRows) {
      ...
      try {
        await Order.create({
          date: row["PO DATE"] ? new Date(row["PO DATE"]) : null,
          brandName: row["PRODUCTS"],
          ...
          section: row["TYPE"],
          productStatus: row["REP/NEW"] || "repeat",
        ...
        console.log(`✅ Imported Row ${rowIndex}: Brand - ${row["PRODUCTS"]}, Client - ${row["PARTY NAMES"]}`);
      } catch (err) {
