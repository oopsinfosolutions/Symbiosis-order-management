...
useEffect(() => {
  axios.get("http://192.168.0.111:5000/api/printers")
    .then((res) => {
  ...
  const handleDownload = async () => {
    try {
      const fileUrl = `http://192.168.0.111:5000/uploads/${formData.attachApprovedArtwork}`;
...
<CreatableSelect
  ...
  onCreateOption={async (inputValue) => {
    try {
      await axios.post('http://192.168.0.111:5000/api/printers/add', { name: inputValue });
      ...
      {showArtwork && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded shadow-xl relative max-w-2xl">
            <img
              src={`http://192.168.0.111:5000/uploads/${formData.attachApprovedArtwork}`}
              alt="Artwork Preview"
