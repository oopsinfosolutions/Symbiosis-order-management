...
const stageFilter = stageParam?.includes(",")
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        if (category !== 'view-all-orders') {
          const res = await axios.get(`http://192.168.0.111:5000/api/orders/by-concerned/${empId}?page=${page}&limit=${limit}`);
          ...
          const [pagedRes, allRes] = await Promise.all([
            ...
            axios.get("http://192.168.0.111:5000/api/orders")
          ]);
    ...
    const fetchPrinters = async () => {
      try {
        const res = await axios.get("http://192.168.0.111:5000/api/printers");
    ...
    const fetchSections = async () => {
      try {
        const res = await axios.get("http://192.168.0.111:5000/api/sections");
  ...
  const handleComplete = async (order) => {
    ...
    try {
      const res = await axios.put(`http://192.168.0.111:5000/api/orders/update-stage`, {
        orderId: order.id,
            ...
            <table className="min-w-full bg-white border orders-table">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="py-2 px-4 border">Concerned Person</th>
                ...
                {orders.map((order) => {
                  ...
                  return (
                    <tr key={order._id}>
                      ...
                      <td className="py-2 px-4 border">{order.concernedPerson}</td>
                      <td className="py-2 px-4 border">{order.clientName}</td>
