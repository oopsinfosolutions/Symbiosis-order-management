...
const stageFilter = stageParam?.includes(",")
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        if (category !== 'view-all-orders') {
          const res = await axios.get(`http://192.168.0.91:5000/api/orders/by-concerned/${empId}?page=${page}&limit=${limit}`);
          ...
          const [pagedRes, allRes] = await Promise.all([
            axios.get(`http://192.168.0.91:5000/api/orders?page=${page}&limit=${limit}`),
            axios.get("http://192.168.0.91:5000/api/orders")
          ]);
    ...
    const fetchPrinters = async () => {
      try {
        const res = await axios.get("http://192.168.0.91:5000/api/printers");
    ...
    const fetchSections = async () => {
      try {
        const res = await axios.get("http://192.168.0.91:5000/api/sections");
  ...
  const handleComplete = async (order) => {
    ...
    try {
      const res = await axios.put(`http://192.168.0.91:5000/api/orders/update-stage`, {
        orderId: order.id,
  ...
  const handleEditOrder = (order) => {
    ...
    axios
      .post("http://192.168.0.91:5000/api/orders/edit-status", {
        orderId: id,
  ...
  try {
    const response = await axios.get("http://192.168.0.91:5000/api/orders", {
      params: {
        empId,
        searchTerm,
        navStatus,
