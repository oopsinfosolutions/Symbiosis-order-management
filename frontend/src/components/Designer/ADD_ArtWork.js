...
const ADDArtWork = () => {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.get(`http://192.168.0.55:5000/api/orders/${id}`);
      // setFormData(prev => ({ ...prev, ...res.data }));
      setFormData(prev => ({ ...prev, ...res.data, concernedPerson: String(res.data.concernedPerson || '') }));

      
      if (res.data.artworkFile) {
        setArtworkPreview(`http://192.168.0.55:5000/uploads/${res.data.artworkFile}`);
      }
    } catch (err) {
  ...
  const fetchBrands = useCallback(async () => {
    try {
      const res = await axios.get("http://192.168.0.55:5000/api/brands");
  ...
  useEffect(() => {
    if (order) {
      ...
      if (order.artworkFile) {
        setArtworkPreview(`http://192.168.0.55:5000/uploads/${order.artworkFile}`);
      }
    } else if (id) {
      fetchOrder();
    }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.post("http://192.168.0.55:5000/api/saveProgress", data, {
